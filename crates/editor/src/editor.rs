...
impl Editor {
        ...
        F: Fn(usize, usize) -> String,
    {
        self.hide_mouse_cursor(HideMouseCursorOrigin::TypingAction, cx);
        let display_snapshot = self.display_snapshot(cx);
        let mut selections = self.selections.all::<usize>(&display_snapshot);
        ...
        let (edits, new_selections) = {
            ...
            let edits: Vec<_> = selections
                ...
                .map(|(selection_order, selection)| {
                    ...
                    let old_len = (selection.end - selection.start) as i64;
                    let adjusted_start = (selection.start as i64 + offset_delta) as usize;
                    let adjusted_end = (adjusted_start as i64 + number_len) as usize;
    ...
    fn modify_number(&mut self, window: &mut Window, cx: &mut Context<Self>, delta: i64) {
        ...
        for selection in selections {
            let (number_range, new_value) = match self.find_and_modify_number_at_selection(
                ...
                None => {
                    let adjusted_start = (selection.start as i64 + offset_delta) as usize;
                    let adjusted_end = (selection.end as i64 + offset_delta) as usize;
            ...
            let adjusted_start = (number_range.start as i64 + offset_delta) as usize;
            let adjusted_end = (adjusted_start as i64 + new_value_len) as usize;

            edits.push((number_range, new_value));
